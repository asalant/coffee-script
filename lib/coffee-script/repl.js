...
(function() {
  var ACCESSOR, CoffeeScript, Module, REPLServer, REPL_PROMPT, REPL_PROMPT_CONTINUATION, REPL_PROMPT_MULTILINE, SIMPLEVAR, Script, autocomplete, completeAttribute, completeVariable, enableColours, getCompletions, inspect, readline,
  ...
  process.on('uncaughtException', function(err) {
    return process.stdout.write((err.stack || err.toString()) + '\n');
  });
  ...
  REPLServer = (function() {
    ...
    function REPLServer(options) {
      ...
      multilineMode = false;
      backlog = '';
      this.input = options.input || process.openStdin();
      ...
      this.rli = rli = this.input.readable && this.input.isRaw ? this.createPipedInterface(this.input, this.output) : this.createReadlineInterface(this.input, this.output);
      ...
      rli.on('line', function(buffer) {
        ...
        backlog = '';
        try {
          return _this["eval"](code, global, 'repl', 'repl', function(err, returnValue) {
            if (err != null) {
              _this.error(err);
            } else {
              ...
              rli.output.write("" + (inspect(returnValue, false, 2, enableColours)) + "\n");
            }
            return rli.prompt();
          });
        } catch (err) {
    ...
    REPLServer.prototype.error = function(err) {
      return this.output.write((err.stack || err.toString()) + '\n');
    };

    REPLServer.prototype.createPipedInterface = function(input, output) {
      var pipedInput, pipedInterface,
        _this = this;
      pipedInput = '';
      pipedInterface = {
        prompt: function() {
          return output.write(this._prompt);
        },
    ...
    REPLServer.prototype.createReadlineInterface = function(input, output) {
      var rli;
  ...
  module.exports = {
    ...
    start: function(options) {
      return new REPLServer(options).start();
    }
  };
